...
car1 = Car(0, int(cfg.center_y), cfg.vx, 0, cfg.ax, 0)  # horizonal car
car2 = Car(int(cfg.center_x), 0, 0, cfg.vy, 0, cfg.ay)  # vertical car


while True:  # keeps the game running
    cfg.gameclock.tick(cfg.framerate)  # set the frame rate at 60 fps
    handle_events()  # check if window was closed
    update()  # run the math
    draw_frame()  # pygame/visual component
    print(light1.state, round(car2.vy, 2), car2.shouldbrake('y'))
